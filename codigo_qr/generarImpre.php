...
<body style="padding-top: 70px;">
    ...
    if ($id_equipo !== null) {
        ...
        $sql = "SELECT numero_serie, area, clave, fechaAs FROM form_impresora WHERE Id = $id_equipo"; // Ajusta la consulta según tu base de datos y estructura de tabla
        ...
        if ($result) {
            ...
            if ($row && isset($row['numero_serie'])) {
                ...
                $contenido = "http://11.11.1.39/inventario-serva/invent/impresoras.php?busqueda=$numero_serie&enviar=";
                ...
                echo '<a href="etiquetapdfImpre.php?id=' . $id_equipo . '" download="equipo_info_' . $numero_serie . '.pdf" class="btn btn-primary">Descargar</a>';
                echo '</div>';
                echo '</div>';
                echo '</div>';
            } else {
