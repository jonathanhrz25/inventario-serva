...
// Recopilar datos del formulario - Hand Held
$clave = $_POST['clave'];
$equipo = $_POST['equipo'];
$serie = $_POST['serie'];
$marca = $_POST['marca'];
...
try {
    ...
    $sql1 = "INSERT INTO form_hh (equipo, numero_serie, clave, marca, modelo, 
             ip, area, usuario, estado, cedis, proceso, comentarios_observaciones, status)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt1 = $conn->prepare($sql1);
    $stmt1->bind_param('sssssssssssss', $equipo, $serie, $clave, $marca, $modelo, 
                        $ip, $area, $usuario, $estado, $cedis, $proceso, $comentarios_observaciones, $status);
    $stmt1->execute();
    ...
    if (array_key_exists($cedis, $cedis_tablas)) {
        ...
        $sql2 = "INSERT INTO $tabla_cedis (clave, area, tipo_equipo, marca, modelo, ip) 
                 VALUES (?, ?, ?, ?, ?, ?)";
        $stmt2 = $connEstadisticas->prepare($sql2);
        $stmt2->execute([$clave, $area, $equipo, $marca, $modelo, $ip]);
        ...
        echo "<script> alert('Sus datos han sido registrados en ambas bases de datos');
                window.location ='../invent/handHeld.php';</script>";
    } else {
        ...
        echo "<script> alert('ERROR: CEDIS no válido');
                window.location ='../formulario/formularioHandHeld.php';</script>";
    }
} catch (Exception $e) {
