...
// Recopilar datos del formulario - Switch y Acces Points
$serie = $_POST['serie'];
$clave = $_POST['clave'];
$equipo = $_POST['equipo'];
$estado = $_POST['estado'];
...
try {
    ...
    $sql1 = "INSERT INTO form_switch_aps (numero_serie, clave, equipo, estado, proceso, marca, 
             modelo, usuario, area, cedis, fechaCo, fechaAs, comentarios_observaciones, status)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt1 = $conn->prepare($sql1);
    $stmt1->bind_param('ssssssssssssssi', $serie, $clave, $equipo, $estado, $proceso, 
    ...
    if (array_key_exists($cedis, $cedis_tablas)) {
        ...
        echo "<script> alert('Sus datos han sido registrados en ambas bases de datos');
                window.location ='../invent/switchesyaccessp.php';</script>";
    } else {
        ...
        echo "<script> alert('ERROR: CEDIS no válido');
                window.location ='../formulario/formularioSwitch_Access.php';</script>";
    }
} catch (Exception $e) {
