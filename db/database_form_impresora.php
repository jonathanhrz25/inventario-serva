...
// Recopilar datos del formulario - Impresora
$clave = $_POST['clave'];
$equipo = $_POST['equipo'];
$serie = $_POST['serie'];
$estado = $_POST['estado'];
...
try {
    ...
    $sql1 = "INSERT INTO form_impresora (equipo, numero_serie, clave, estado, proceso, marca, 
             modelo, especificaciones, usuario, area, cedis, fechaCo, fechaAs, comentarios_observaciones, status)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt1 = $conn->prepare($sql1);
    $stmt1->bind_param('ssssssssssssssi', $equipo, $serie, $clave, $estado, $proceso, $marca, 
                        $modelo, $especificaciones, $usuario, $area, $cedis, $fechaCo, $fechaAs, 
                        $comentarios_observaciones, $status);
    $stmt1->execute();
    ...
    if (array_key_exists($cedis, $cedis_tablas)) {
        ...
        echo "<script> alert('Sus datos han sido registrados en ambas bases de datos');
                window.location ='../invent/impresoras.php';</script>";
    } else {
        ...
        echo "<script> alert('ERROR: CEDIS no válido');
                window.location ='../formulario/formularioImpresora.php';</script>";
    }
} catch (Exception $e) {
