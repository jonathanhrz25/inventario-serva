...
// Recopilar datos del formulario - Laptops
$equipo = $_POST['equipo'];
$serie = $_POST['serie'];
$clave = $_POST['clave'];
$estado = $_POST['estado'];
...
try {
    ...
    $sql1 = "INSERT INTO form_laptop (equipo, numero_serie, clave, estado, proceso, marca, 
             modelo, especificaciones, usuario, area, cedis, responsiva, fechaCo, fechaAs, comentarios_observaciones, status)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt1 = $conn->prepare($sql1);
    $stmt1->bind_param('sssssssssssssssi', $equipo, $serie, $clave, $estado, $proceso, 
                        $marca, $modelo, $especificaciones, $usuario, $area, $cedis, 
                        $responsiva, $fechaCo, $fechaAs, $comentarios_observaciones, $status);
    $stmt1->execute();
    ...
    if (array_key_exists($cedis, $cedis_tablas)) {
        ...
        $sql2 = "INSERT INTO $tabla_cedis (clave, area, tipo_equipo, marca, modelo, especificaciones) 
                 VALUES (?, ?, ?, ?, ?, ?)";
        $stmt2 = $connEstadisticas->prepare($sql2);
        $stmt2->execute([$clave, $area, $equipo, $marca, $modelo, $especificaciones]);
        ...
        echo "<script> alert('Sus datos han sido registrados en ambas bases de datos');
                window.location ='../invent/laptops.php';</script>";
    } else {
        ...
        echo "<script> alert('ERROR: CEDIS no válido');
                window.location ='../formulario/formularioLaptop.php';</script>";
    }
} catch (Exception $e) {
