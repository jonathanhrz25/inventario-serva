...
// Recopilar datos del formulario - Lector CB
$equipo = $_POST['equipo'];
...
try {
    ...
    $sql1 = "INSERT INTO form_lectorcb (equipo, numero_serie, clave, estado, marca, modelo, 
             usuario, area, cedis, especificaciones, fechaCo, fechaAs, comentarios_observaciones, status)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt1 = $conn->prepare($sql1);
    $stmt1->bind_param('ssssssssssssssi', $equipo, $numero_serie, $clave, $estado, $marca, 
                        $modelo, $usuario, $area, $cedis, $especificaciones, 
    ...
    if (array_key_exists($cedis, $cedis_tablas)) {
        ...
        echo "<script> alert('Sus datos han sido registrados en ambas bases de datos');
                window.location ='../invent/lectorCB.php';</script>";
    } else {
        ...
        echo "<script> alert('ERROR: CEDIS no válido');
                window.location ='../formulario/formularioLectorCB.php';</script>";
    }
} catch (Exception $e) {
