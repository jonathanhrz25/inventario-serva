...
// Recopilar datos del formulario - Monitor
$clave = $_POST['clave'];
$equipo = $_POST['equipo'];
$serie = $_POST['serie'];
$estado = $_POST['estado'];
...
try {
    ...
    $sql1 = "INSERT INTO form_monitor (equipo, numero_serie, clave, estado, proceso, marca, modelo, 
             tipo_pantalla, tamaño_pantalla, usuario, area, cedis, fechaCo, fechaAs, comentarios_observaciones, status)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt1 = $conn->prepare($sql1);
    $stmt1->bind_param('sssssssssssssssi', $equipo, $serie, $clave, $estado, $proceso, $marca, $modelo, 
                        $tipo_pantalla, $tamaño_pantalla, $usuario, $area, $cedis, $fechaCo, $fechaAs, $comentarios_observaciones, $status);
    $stmt1->execute();
    ...
    if (array_key_exists($cedis, $cedis_tablas)) {
        ...
        $sql2 = "INSERT INTO $tabla_cedis (clave, area, tipo_equipo, marca, modelo, tipo_pantalla, tamaño_pantalla) 
                 VALUES (?, ?, ?, ?, ?, ?, ?)";
        $stmt2 = $connEstadisticas->prepare($sql2);
        $stmt2->execute([$clave, $area, $equipo, $marca, $modelo, $tipo_pantalla, $tamaño_pantalla]);
        ...
        echo "<script> alert('Sus datos han sido registrados en ambas bases de datos');
                window.location ='../invent/monitores.php';</script>";
    } else {
        ...
        echo "<script> alert('ERROR: CEDIS no válido');
                window.location ='../formulario/formularioMonitor.php';</script>";
    }
} catch (Exception $e) {
