...
// Recopilar datos del formulario - Pantalla
$clave = $_POST['clave'];
$equipo = $_POST['equipo'];
$serie = $_POST['serie'];
$estado = $_POST['estado'];
...
try {
    ...
    $sql1 = "INSERT INTO form_pantalla (equipo, numero_serie, clave, estado, proceso, marca, 
             modelo, usuario, area, fechaCo, fechaAs, comentarios_observaciones, status)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt1 = $conn->prepare($sql1);
    $stmt1->bind_param('sssssssssssss', $equipo, $serie, $clave, $estado, $proceso, 
                        $marca, $modelo, $usuario, $area, $fechaCo, $fechaAs, 
    ...
    if (array_key_exists($cedis, $cedis_tablas)) {
        ...
        $sql2 = "INSERT INTO $tabla_cedis (clave, area, tipo_equipo, marca, modelo) 
                 VALUES (?, ?, ?, ?, ?)";
        $stmt2 = $connEstadisticas->prepare($sql2);
        $stmt2->execute([$clave, $area, $equipo, $marca, $modelo]);
        ...
        echo "<script> alert('Sus datos han sido registrados en ambas bases de datos');
                window.location ='../invent/pantallas.php';</script>";
    } else {
        ...
        echo "<script> alert('ERROR: CEDIS no válido');
                window.location ='../formulario/formularioPantalla.php';</script>";
    }
} catch (Exception $e) {
