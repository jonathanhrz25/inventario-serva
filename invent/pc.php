...
<body>
    <header>
        <nav class="navbar navbar-expand-lg navbar-dark bg-dark fixed-top" style="background-color: #081856!important;">
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                ...
                <a class="btn btn-success ml-2" href="../formulario/formularioPC.php">Añadir</a>
            </div>
        </nav>
    ...
    <h1 class="display-6">Tabla de Inventario - PC's</h1>
        ...
        <div class="table-responsive">
            <table class="table table-hover">
                ...
                ?>
                    <th><a class="text-white" href="?order_by=Id&order=<?php echo $new_order; ?>&busqueda=<?php echo urlencode(isset($_GET['busqueda']) ? $_GET['busqueda'] : ''); ?>">Id</a></th>
                    <th>
                        <?php if ($order_by === 'clave'): ?>
                            <a  href="http://localhost/inventario-serva/invent/pc.php?busqueda=&enviar=">Clave <i class="bi bi-sort-up-alt"></i></a>
                        <?php else: ?>
                            <a class="text-white" href="?order_by=clave&order=<?php echo $order; ?>&busqueda=<?php echo urlencode(isset($_GET['busqueda']) ? $_GET['busqueda'] : ''); ?>">Clave <i class="bi bi-sort-down"></i></a>
                        <?php endif; ?>
                    </th>
                    <th>Equipo</th>
            ...
            <?php
                ...
                $sql_paginacion = "SELECT * FROM form_pc WHERE 
            ...
            ?>
                        <tr>
                            ...
                            <td class="text-center"><?php echo $mostrar['responsiva']; ?></td>
                            ...
                            <td class="text-center"><?php echo $mostrar['proceso']; ?></td>
                            ...
                            <td class="text-center icon-cell"><a href="../codigo_qr/generarPC.php?id=<?php echo $mostrar['Id']; ?>"><i class="fa fa-qrcode fa-2x"></i></a></td>
                            <td class="text-center icon-cell"><a href="../edit/modificarPC.php?id=<?php echo $mostrar['Id']; ?>"><i class="fa fa-pencil-square-o fa-2x"></i></a></td>
                            <td class="text-center icon-cell"><a href="../edit/eliminarPC.php?id=<?php echo $mostrar['Id']; ?>" onclick="return confirm('¿Estás seguro?')"><i class="fa fa-trash-o fa-2x"></i></a></td>
                        </tr>
                    <?php }
                } else {
    ...
    $sql_total_resultados = "SELECT COUNT(*) as total FROM form_pc WHERE 
        (clave LIKE '%$busqueda%' OR numero_serie LIKE '%$busqueda%' OR area LIKE '%$busqueda%' 
        OR usuario LIKE '%$busqueda%' OR cedis LIKE '%$busqueda%') AND status = 1";
    ...
    <nav aria-label="Paginación de inventario">
        <ul class="pagination">
            ...
            <li class="page-item <?php echo ($pagina_actual <= 1) ? 'disabled' : ''; ?>">
                <a class="page-link" href="?pagina=<?php echo ($pagina_actual <= 1) ? 1 : ($pagina_actual - 1); ?>&order_by=<?php echo $order_by; ?>&order=<?php echo $order; ?>&busqueda=<?php echo urlencode($busqueda); ?>">Anterior</a>
            </li>
            ...
            <li class="page-item <?php echo ($pagina_actual >= $total_paginas) ? 'disabled' : ''; ?>">
                <a class="page-link" href="?pagina=<?php echo ($pagina_actual >= $total_paginas) ? $total_paginas : ($pagina_actual + 1); ?>&order_by=<?php echo $order_by; ?>&order=<?php echo $order; ?>&busqueda=<?php echo urlencode($busqueda); ?>">Siguiente</a>
            </li>
        </ul>
    </nav>
</div><br><br>
