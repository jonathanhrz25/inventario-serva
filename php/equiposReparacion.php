...
<head>
    ...
    <h1 class="display-5">Equipos en Reparacion</h1>
</head>
...
<body>
    ...
    <div id="info-container" class="info-container">
        ...
        if (isset($_GET['enviar'])) {
            ...
            if (isset($conn) && !is_null($conn)) {
                ...
                if ($tipo === 'PC') {
                    $sql = "SELECT * FROM form_pc WHERE proceso = 'Reparacion'";
                    $stmt = mysqli_prepare($conn, $sql);

                } elseif ($tipo === 'Monitor') {
                    $sql = "SELECT * FROM form_monitor WHERE proceso = 'Reparacion'";
                    $stmt = mysqli_prepare($conn, $sql);

                } elseif ($tipo === 'Impresora') {
                    $sql = "SELECT * FROM form_impresora WHERE proceso = 'Reparacion'";
                    $stmt = mysqli_prepare($conn, $sql);

                } elseif ($tipo === 'Hand Held') {
                    $sql = "SELECT * FROM form_hh WHERE proceso = 'Reparacion'";
                    $stmt = mysqli_prepare($conn, $sql);

                } elseif ($tipo === 'Pantalla') {
                    $sql = "SELECT * FROM form_pantalla WHERE proceso = 'Reparacion'";
                    $stmt = mysqli_prepare($conn, $sql);

                } elseif ($tipo === 'Laptop') {
                    $sql = "SELECT * FROM form_laptop WHERE proceso = 'Reparacion'";
                    $stmt = mysqli_prepare($conn, $sql);

                } elseif ($tipo === 'NoBreak') {
                    $sql = "SELECT * FROM form_nobreak WHERE proceso = 'Reparacion'";
                    $stmt = mysqli_prepare($conn, $sql);

                } elseif ($tipo === 'Switches y Access Points') {
                    $sql = "SELECT * FROM form_switch_aps WHERE proceso = 'Reparacion'";
                    $stmt = mysqli_prepare($conn, $sql);

                } elseif ($tipo === 'Disco Duro') {
                    $sql = "SELECT * FROM form_dde WHERE proceso = 'Reparacion'";
                    $stmt = mysqli_prepare($conn, $sql);
    ...
    <script>
        function cargarContenido(tipo) {
            ...
            if (tipo === 'PC') {
                url = '../reparacion/reparacionPC.php';
            } else if (tipo === 'Monitor') {
                url = '../reparacion/reparacionMonit.php';
            } else if (tipo === 'Impresora') {
                url = '../reparacion/reparacionImpre.php';
            } else if (tipo === 'Hand Held') {
                url = '../reparacion/reparacionHH.php';
            } else if (tipo === 'Pantalla') {
                url = '../reparacion/reparacionPant.php';
            } else if (tipo === 'Laptop') {
                url = '../reparacion/reparacionLap.php';
            } else if (tipo === 'NoBreak') {
                url = '../reparacion/reparacionNoBreak.php';
            } else if (tipo === 'Switches y Access Points') {
                url = '../reparacion/reparacionSyA.php';
            } else if (tipo === 'Disco Duro') {
                url = '../reparacion/reparacionDDE.php';
            }
